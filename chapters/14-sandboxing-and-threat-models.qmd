...
- Give it secrets, and it can accidentally paste them into a prompt—or intentionally exfiltrate them if it’s been prompt-injected.

Part II showed how to build a useful workflow. This part is about making that power *safe to deploy*.

In security terms: we’re going to reduce attack surface, enforce least privilege, and put hard isolation boundaries around untrusted execution—so that when something goes wrong, it fails safely.
...
3. **Container isolation** (Docker / cloud): untrusted code that can write files and run programs is still confined to an ephemeral environment.
